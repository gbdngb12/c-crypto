...
void x9_17_random_generator(byte *random_number, byte *key1, byte *key2, int random_number_size);
// 날짜와 시간을 얻는 함수
void get_date_time(byte *dt);
// 초기 벡터 값 설정 함수
void set_vector(byte *v);
// 삼중 DES 암호화 함수
...
int main() {
    int i, j;
    byte random_number[16] = { 0 };
    byte key1[BLOCK_SIZE + 1] = "infosec1";
    byte key2[BLOCK_SIZE + 1] = "security";

    for(j = 0; j < 10; j++) {
        x9_17_random_generator(random_number, key1, key2, 16);
...
void x9_17_random_generator(byte *random_number, byte *key1, byte *key2, int random_number_size) {
    ...
    for (i = 0; i < random_number_size; i += BLOCK_SIZE) {
        get_date_time(dt);
