...
static void padding(byte *, uint64);
static void md_5_init();
static void md_5_digest(byte *);
static void md_5(FILE *, byte *);
static void ff(uint* a,uint b, uint c, uint d, uint _m, int s, uint t);
static void gg(uint* a,uint b, uint c, uint d, uint _m, int s, uint t);
static void hh(uint* a,uint b, uint c, uint d, uint _m, int s, uint t);
static void ii(uint* a,uint b, uint c, uint d, uint _m, int s, uint t);

#define byte_to_word(a,b,c,d) ( ((a << 24) + (b << 16) + (c << 8) + d) )
#define circular_shift(X,n) ( ((X) << n) | ((X) >> (32-n)) )			

static void make_bit_128(byte[16],uint , uint , uint , uint);//word단위 해쉬 값을 byte단위로 변환 하는 함수

void ff(uint* a,uint b, uint c, uint d, uint _m, int s, uint t) {
    *a = b + circular_shift((*a + F(b,c,d) + _m + t), s);
}

void gg(uint* a,uint b, uint c, uint d, uint _m, int s, uint t) {
    *a = b + circular_shift((*a + G(b,c,d) + _m + t), s);
}

void hh(uint* a,uint b, uint c, uint d, uint _m, int s, uint t) {
    *a = b + circular_shift((*a + H(b,c,d) + _m + t), s);
}

void ii(uint* a,uint b, uint c, uint d, uint _m, int s, uint t) {
    *a = b + circular_shift((*a + I(b,c,d) + _m + t), s);
}
