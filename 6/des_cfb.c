...
// des_cfb Mode 암호화 함수
void des_cfb_encryption(byte *message, byte *result, byte *iv, byte *key, int msg_len);
// des_cfb Mode 복호화 함수
void des_cfb_decryption(byte *message, byte *result, byte *iv, byte *key, int msg_len);
...
int main() {
    ...
    des_cfb_encryption(plain_text, cipher_text, iv, key, msg_len);//des_cfb 암호화
    ...
    des_cfb_decryption(cipher_text, decrypt_text, iv, key, msg_len); //des_cfb 복호화
    printf("\nDecrypt Text: ");
    for (i = 0; i < msg_len; i++) {
...
void des_cfb_encryption(byte *message/*plain_text*/, byte *result/*cipher_text*/, byte *iv, byte *key, int msg_len) {
    int i, j;
    ...
    for(i = 0; i < msg_len; i+= BLOCK_SIZE) {
        ...
        for(j = 0; j < BLOCK_SIZE; j++) {
            vector[j] = result[i + j]; //다음 초기벡터는 C_0
        }
    }
}

void des_cfb_decryption(byte *message/*cipher_text*/, byte *result/*decrypt_text*/, byte *iv, byte *key, int msg_len) {
    int i, j;
    byte vector[BLOCK_SIZE] = { 0 };
