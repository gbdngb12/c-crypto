...
// DES_CBC Mode 암호화 함수
void des_cbc_encryption(byte *message, byte *result, byte *iv, byte *key, int msg_len);
// DES_CBC Mode 복호화 함수
void des_cbc_decryption(byte *message, byte *result, byte *iv, byte *key, int msg_len);
...
int main() {
    ...
    des_cbc_encryption(plain_text, cipher_text, iv, key, msg_len);//DES_CBC 암호화
    ...
    des_cbc_decryption(cipher_text, decrypt_text, iv, key, msg_len); //DES_CBC 복호화
    printf("\nDecrypt Text: ");
    for (i = 0; i < msg_len; i++) {
...
void des_cbc_encryption(byte *message/*plain_text*/, byte *result/*cipher_text*/, byte *iv, byte *key, int msg_len) {
    int i, j;
    ...
    for(i = 0; i < msg_len; i+= BLOCK_SIZE) {
        byte plain_text[BLOCK_SIZE] = { 0 };

        for(j = 0; j < BLOCK_SIZE; j++) {
            plain_text[j] = message[i + j] ^ vector[j];// P_0 ^ IV, C_0 ^ P_1
        }

        //C_0(result) = E(P_0 ^ IV, K)
        //C_1(result) = E(C_0 ^ P_1, K)
        des_encryption(plain_text,&result[i]/**/, key);
...
void des_cbc_decryption(byte *message/*cipher_text*/, byte *result/*decrypt_text*/, byte *iv, byte *key, int msg_len) {
    int i, j;
    byte vector[BLOCK_SIZE] = { 0 };
    ...
    for(i = 0; i < msg_len; i+=BLOCK_SIZE) {
        byte decrypt_text[BLOCK_SIZE] = { 0 };

        des_decryption(&message[i],decrypt_text,key);

        //i > 0 인경우에는 C_0, C_1, ... , C_{n-1}과 xor
        if(i > 0) {
            for(j = 0; j < BLOCK_SIZE; j++) {
                vector[j] = message[i + j - BLOCK_SIZE/*8, 1Block*/]; /*Cipher_text*/
            }
        }
        //i == 0 인경우에는 iv와 xor
        for(j = 0; j < BLOCK_SIZE; j++) {
            result[i + j] = decrypt_text[j] ^ vector[j];
        }
    }
}