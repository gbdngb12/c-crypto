...
#define AES_Nb 4 //AES 평문 워드수(4word) (128비트 / 32비트(word)-> 4word) 
#define AES_Nk 4 //AES 키의 워드수(4word) (128비트 / 32비트(word)-> 4word)
...
void aes_add_round_key(byte state[][4], word*);//AddRoundKey
void aes_key_expansion(byte *key, word* w);//AES 키 확장 함수
//이거 뭔가 잘못된듯
void aes_circular_shift_rows(byte* row); //state의 한행을 1회 오른쪽으로 순환 시프트
void aes_inverse_circular_shift_rows(byte *row);//state의 한행을 1회 왼쪽으로 순환 시프트
word aes_sub_word(word w); //SubWord
word aes_rot_word(word w); //RotWord
byte aes_x_time(byte n, byte b); //GF(2^8) 상에서 곱셈 연산 함수
...
//암호화 s-box
static byte aes_s_box_table[16][16] = {
...
//복호화 s-box
static byte aes_inverse_s_box_table[16][16] = {
...
//Rcon 상수
static word aes_Rcon[11] = {0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000,
                        0x20000000, 0x40000000, 0x80000000, 0x1b000000, 0x36000000};

static int aes_number_of_round;//라운드 수
